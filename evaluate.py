...
with torch.no_grad():
    for batch in tqdm(test_loader, desc="Evaluating"):
        ...
        # Generate probability distribution plots for first batch
        import matplotlib.pyplot as plt
        import numpy as np
        # Sample 10 random indices from the batch
        random_indices = np.random.choice(len(batch['input_ids']), min(10, len(batch['input_ids'])), replace=False)
        for idx in random_indices:
            logit = logits[idx]  # Get individual prediction
            softmaxed_dist = torch.softmax(logit, dim=0)  # Apply softmax
            
            # Create figure with scientific styling
            plt.figure(figsize=(12, 6))
            
            # Plot probability distribution
            x_values = [i * 100 + 50 for i in range(len(softmaxed_dist))]
            softmaxed_np = softmaxed_dist.cpu().numpy()
            plt.plot(x_values, softmaxed_np, linewidth=2, color='#2E86C1')
            
            # Add shaded area under curve
            plt.fill_between(x_values, softmaxed_np, alpha=0.2, color='#2E86C1')
            
            # Calculate statistics
            mean = np.sum(np.array(x_values) * softmaxed_np)
            std = np.sqrt(np.sum(softmaxed_np * (np.array(x_values) - mean)**2))
            
            
            # Customize plot
            plt.xlabel('Electron Temperature (K)', fontsize=12)
            plt.ylabel('Probability', fontsize=12)
            plt.title(f'Probability Distribution of Prediction (Normal Period)',
                        fontsize=14, pad=20)
            ...
            plt.ylim(0, max(softmaxed_np) * 1.1)
            
            # Adjust layout and save
            plt.tight_layout()
            plt.savefig(f'./normal_entropy_{idx}.png', dpi=300, bbox_inches='tight')
            plt.close()
