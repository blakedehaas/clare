...
\begin{enumerate}
    ...
    \item \textbf{Contiguous Validation Blocks (\texttt{val-normal}):} Non-storm telemetry is partitioned into equal-sized contiguous orbital blocks ($B = 150$ measurements, nominally representing one complete $\sim 2.5$-hour orbital pass through the $1{,}000$--$8{,}000$\,km altitude window). Partitioning is performed by fixed sample count rather than clock time to avoid empty or ragged windows resulting from ground-station tracking gaps, ensuring that every evaluation block maintains identical degrees of freedom ($N = 150$). 334 non-adjacent blocks ($50{,}100$ samples) are isolated for early stopping, learning rate scheduling, and Bayesian hyperparameter tuning.
    ...
    \item \textbf{Training Chunks (\texttt{train\_chunks}):} The remaining observations ($\sim 3.2$\,million non-storm measurements) are used to optimize model weights, with adjacent guard-band blocks ($100{,}200$ samples) permanently withheld to eliminate boundary autocorrelation.
\end{enumerate}
